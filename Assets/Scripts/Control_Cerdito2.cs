...
public class Control_Cerdito2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //ACCERELATION
        if(Input.GetKey(KeyCode.W) && Speed<MaxSpeed && countdown.movement==true  )
        {
        ...
        if(Input.GetKey(KeyCode.S)&& countdown.movement==true  )
        {
            Back_Left.brakeTorque=0;
            Back_Right.brakeTorque=0;
            Back_Left.motorTorque=Input.GetAxis("Vertical")*Torque*CoefAccelaration*Time.deltaTime;
        ...
        if(Input.GetKeyUp(KeyCode.W) || Speed>MaxSpeed || Input.GetKeyUp(KeyCode.S))

        {
            ...
            if (!Input.GetKey(KeyCode.W) && (Speed > 1 ) )//hola para activar frenado 
            {
